...
def get_window_scale_pct() -> float:
    try:
        pct = float(ProgramSettings.get_setting("WINDOW_SCALE_PCT")) / 100.0
        if not (0 < pct <= 1):
            pct = GuiSettings.get_default_scale_percent()
    except Exception:
        pct = GuiSettings.get_default_scale_percent()
    return pct
...
def main():
    ...
    pct_scaling = get_window_scale_pct()
    ...
    window = ImageWindow(
        image_path, title, gui_settings.scaled_width, gui_settings.scaled_height
    )
